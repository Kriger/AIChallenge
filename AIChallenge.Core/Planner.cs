...
public class Planner
{
    ...
    public async Task<Plan?> CreatePlanAsync(string request, List<Fact>? relevantFacts = null, List<string>? invariants = null)
    {
        try
        {
            ...
            var messages = new List<AIChallenge.Models.ApiMessage>
            {
    ...
    public async Task<Plan> ExecutePlanAsync(Plan plan, List<Fact>? relevantFacts = null)
    {
        ...
        {
            task.Status = AIChallenge.Models.TaskStatus.Running;
            ...
            try
            {
                ...
                task.Result = result;
                task.Status = AIChallenge.Models.TaskStatus.Completed;
            ...
            {
                task.Status = AIChallenge.Models.TaskStatus.Failed;
                task.CompletedAt = DateTime.UtcNow;
    ...
    private string GetPreviousResults(int currentTaskId)
    {
        ...
        foreach (var task in _plansHistory.Values.Where(p => p.Tasks.Any(t => t.Id < currentTaskId && t.Status == AIChallenge.Models.TaskStatus.Completed)))
        {
            foreach (var completedTask in task.Tasks.Where(t => t.Id < currentTaskId && t.Status == AIChallenge.Models.TaskStatus.Completed))
            {
    ...
    {
        var completedTasks = plan.Tasks.Where(t => t.Status == AIChallenge.Models.TaskStatus.Completed).ToList();
        var failedTasks = plan.Tasks.Where(t => t.Status == AIChallenge.Models.TaskStatus.Failed).ToList();
