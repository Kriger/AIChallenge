...
public class StickyFactsStrategy : IContextStrategy
{
    ...
    public void UpdateFacts()
    {
        ...
        try
        {
            ...
            {
                var parsed = response.Content.ReadFromJsonAsync<AIChallenge.Models.CompletionResponse>().GetAwaiter().GetResult();
    ...
    private async void UpdateFactsAsync(string userMessage)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var parsed = await response.Content.ReadFromJsonAsync<AIChallenge.Models.CompletionResponse>();
                if (parsed?.Choices?.Count > 0)
                {
                    var factsText = parsed.Choices[0].Message?.Content ?? "";
